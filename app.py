...
# Validated Retail Scarcity Score (RSS) weights.
# RSS uses only documented retail release structure; resale price, return, premium,
# sales volume, brand prestige, and cultural status are excluded from the calculation.
RSS_D_WEIGHT = 0.45
RSS_A_WEIGHT = 0.20
RSS_R_WEIGHT = 0.35

DISTRIBUTION_RUBRIC = {
    100: "100 — One retail organization / ecosystem",
    75: "75 — Brand + select specialty / Tier-Zero / luxury stores",
    50: "50 — Select release including some larger retailers",
    25: "25 — Broad major-retailer release",
    0: "0 — Ongoing broad / core product",
}

ACCESS_RUBRIC = {
    100: "100 — Closed access / EA / invite / extremely restricted",
    75: "75 — Raffle / draw / password primary",
    50: "50 — Mix of controlled + normal access",
    25: "25 — Normal scheduled limited drop / FCFS",
    0: "0 — Ordinary ongoing checkout",
}

REPLENISHMENT_RUBRIC = {
    100: "100 — No later authorized reissue",
    75: "75 — One later re-release",
    50: "50 — 2–3 later releases",
    25: "25 — 4+ windows / recurring equivalent product",
    0: "0 — Permanent / continuous product",
}

def calculate_retail_scarcity_score(
    distribution_restriction: float,
    access_restriction: float,
    replenishment_scarcity: float | None,
) -> float:
    """Validated RSS formula with renormalization when replenishment is unobservable."""
    numerator = RSS_D_WEIGHT * float(distribution_restriction) + RSS_A_WEIGHT * float(access_restriction)
    denominator = RSS_D_WEIGHT + RSS_A_WEIGHT
    if replenishment_scarcity is not None and not pd.isna(replenishment_scarcity):
        numerator += RSS_R_WEIGHT * float(replenishment_scarcity)
        denominator += RSS_R_WEIGHT
    if denominator <= 0:
        return 0.0
...
def load_market() -> pd.DataFrame:
    ...
    # Backward compatibility: older GitHub datasets used `scarcity_score`.
    # New production data uses `retail_scarcity_score` as the canonical field.
    if "retail_scarcity_score" not in df.columns and "scarcity_score" in df.columns:
        df["retail_scarcity_score"] = df["scarcity_score"]

    numeric_cols = [
        "retail_price", "release_year", "current_resale", "price_low", "price_high",
        "volatility_pct", "sales_volume", "distribution_restriction",
        "access_restriction", "replenishment_scarcity", "retail_scarcity_score",
        "legacy_scarcity_score", "item_cagr", "matched_sp_cagr", "excess_cagr",
    ]
    for col in numeric_cols:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    # Ensure the canonical RSS field exists even on older/local test files.
    if "retail_scarcity_score" not in df.columns:
        df["retail_scarcity_score"] = np.nan
...
def add_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["investability_score"] = (
        ...
        + 0.15 * np.clip(out["retail_scarcity_score"], 0, 100)
    ).clip(0, 100)
    ...
    scarcity = np.clip(out["retail_scarcity_score"], 0, 100)
    ...
    for _, row in market.iterrows():
        ...
        for month, price in zip(dates, series):
            rows.append({
                ...
                "retail_scarcity_score": row["retail_scarcity_score"],
...
def fit_regression(panel: pd.DataFrame):
    ...
    numeric = ["retail_price", "retail_scarcity_score", "grail_score", "sale_count", "avg_size"]
...
def custom_scores(
    ...
    # Auto-estimate volatility from recent low/high range. Scarcity is NOT auto-estimated
    # from market price, sales volume, age, or collaboration status.
    ...
    retail_scarcity_score = calculate_retail_scarcity_score(
        distribution_restriction=distribution_restriction,
        access_restriction=access_restriction,
        replenishment_scarcity=replenishment_scarcity,
    )
    ...
    # Grail score is a separate collector/cultural construct. RSS itself remains independent
    # of premium, liquidity, brand, and cultural boosts.
    grail_score = (
        0.30 * premium_score
        + 0.25 * retail_scarcity_score
        + 0.20 * age_score
        + 0.15 * low_volume_score
        + cultural_boost
    )
    ...
    return {
        ...
        "distribution_restriction": float(distribution_restriction),
        "access_restriction": float(access_restriction),
        "replenishment_scarcity": None if replenishment_scarcity is None else float(replenishment_scarcity),
        "replenishment_status": "Withheld" if replenishment_scarcity is None else "Observed",
        "retail_scarcity_score": retail_scarcity_score,
...
def stockx_import_defaults(url: str, market: pd.DataFrame) -> dict:
    ...
    if match_row is not None and match_score >= 0.72:
        ...
        return {
            **inferred,
            ...
            "distribution_restriction": int(match_row["distribution_restriction"]) if pd.notna(match_row.get("distribution_restriction")) else 50,
            "access_restriction": int(match_row["access_restriction"]) if pd.notna(match_row.get("access_restriction")) else 25,
            "replenishment_status": "Withheld" if ("withheld" in str(match_row.get("replenishment_status", "")).lower() or pd.isna(match_row.get("replenishment_scarcity"))) else "Observed",
            "replenishment_scarcity": int(match_row["replenishment_scarcity"]) if pd.notna(match_row.get("replenishment_scarcity")) else 50,
    ...
    return {
        **inferred,
        ...
        "release_year": 2023,
        "distribution_restriction": 50,
        "access_restriction": 25,
        "replenishment_status": "Withheld",
        "replenishment_scarcity": 50,
        "match_status": "No local data match. Market numbers use smart defaults; retail-scarcity inputs must be confirmed from release evidence.",
        "matched_item": "",
        "match_score": match_score,
    }
...
FRIENDLY_COLUMNS = {
    ...
    "distribution_restriction": "Distribution Restriction (D)",
    "access_restriction": "Access Restriction (A)",
    "replenishment_scarcity": "Replenishment Scarcity (R)",
    "replenishment_status": "R Status",
    "retail_scarcity_score": "Retail Scarcity Score (RSS)",
    "scarcity_confidence": "RSS Confidence",
    "scarcity_basis": "RSS Evidence Basis",
    "legacy_scarcity_score": "Legacy Scarcity",
    "benchmark_sample": "Clean-70 Sample",
    "excess_cagr": "Excess CAGR vs S&P 500",
...
def investability_scores_for_weights(df: pd.DataFrame, weights: dict[str, float]) -> pd.Series:
    ...
    components = {
        ...
        "Retail Scarcity": df["retail_scarcity_score"].fillna(0),
    }
    total_weight = sum(weights.values())
    if total_weight <= 0:
        weights = {"Liquidity": 0.40, "Premium": 0.25, "Inverse Risk": 0.20, "Retail Scarcity": 0.15}
        total_weight = 1.0
...
def sensitivity_analysis(df: pd.DataFrame, top_n: int = 10, delta: float = 0.10) -> tuple[pd.DataFrame, pd.DataFrame]:
    base_weights = {"Liquidity": 0.40, "Premium": 0.25, "Inverse Risk": 0.20, "Retail Scarcity": 0.15}
    work = df.copy()
...
with tabs[0]:
    ...
    st.write(
        ...
        "It compares resale baskets against S&P 500, Nike, and gold, while also measuring risk, liquidity, premium, collector effects, and a validated Retail Scarcity Score based only on release structure."
    )
...
with tabs[1]:
    ...
    st.write(
        ...
        "The app fills matched local market data when available. Retail Scarcity Score inputs must come from documented release structure, not resale price or sales volume."
    )
    ...
    estimator_defaults = {
        ...
        "est_distribution_restriction": 25,
        "est_access_restriction": 25,
        "est_replenishment_status": "Observed",
        "est_replenishment_scarcity": 0,
        "est_horizon": 12,
    ...
    if fill_from_link:
        ...
        st.session_state["est_distribution_restriction"] = int(defaults.get("distribution_restriction", 50))
        st.session_state["est_access_restriction"] = int(defaults.get("access_restriction", 25))
        st.session_state["est_replenishment_status"] = defaults.get("replenishment_status", "Withheld")
        st.session_state["est_replenishment_scarcity"] = int(defaults.get("replenishment_scarcity", 50))
    ...
    with st.form("unified_estimator_form"):
        ...
        st.markdown("### 3. Retail Scarcity Score inputs")
        st.caption(
            "RSS is based only on documented retail release structure. Price, resale premium, sales volume, brand prestige, and returns do not affect these inputs."
        )
        rss_col1, rss_col2, rss_col3 = st.columns(3)
        with rss_col1:
            distribution_restriction = st.selectbox(
                "Distribution restriction (D)",
                [100, 75, 50, 25, 0],
                format_func=lambda x: DISTRIBUTION_RUBRIC[x],
                key="est_distribution_restriction",
                help="How narrowly the item was distributed at retail."
            )
        with rss_col2:
            access_restriction = st.selectbox(
                "Access restriction (A)",
                [100, 75, 50, 25, 0],
                format_func=lambda x: ACCESS_RUBRIC[x],
                key="est_access_restriction",
                help="How controlled the purchase process was at launch."
            )
        with rss_col3:
            replenishment_status = st.radio(
                "Can replenishment be fairly observed?",
                ["Observed", "Withheld"],
                horizontal=True,
                key="est_replenishment_status",
                help="Use Withheld for products too new to judge. RSS will renormalize D and A rather than guessing R."
            )
            if replenishment_status == "Observed":
                replenishment_scarcity = st.selectbox(
                    "Replenishment scarcity (R)",
                    [100, 75, 50, 25, 0],
                    format_func=lambda x: REPLENISHMENT_RUBRIC[x],
                    key="est_replenishment_scarcity",
                )
            else:
                replenishment_scarcity = None
                st.caption("R is withheld; the 45% D and 20% A weights are renormalized automatically.")

        rss_preview = calculate_retail_scarcity_score(
            distribution_restriction, access_restriction, replenishment_scarcity
        )
        st.info(f"Retail Scarcity Score preview: **{rss_preview:.2f}/100**")

        st.markdown("### 4. Optional history and projection controls")
        h1, h2 = st.columns(2)
        with h1:
        ...
        with st.expander("Advanced volatility override, optional"):
    ...
    if submitted:
        ...
        m6.metric("Retail scarcity", f"{custom['retail_scarcity_score']:.2f}/100")
        ...
        export_row = pd.DataFrame([{
            ...
            "distribution_restriction": custom["distribution_restriction"],
            "access_restriction": custom["access_restriction"],
            "replenishment_scarcity": custom["replenishment_scarcity"],
            "replenishment_status": custom["replenishment_status"],
            "retail_scarcity_score": custom["retail_scarcity_score"],
            "scarcity_score": custom["retail_scarcity_score"],
...
with tabs[4]:
    research_tabs = st.tabs(["Findings", "Retail Scarcity", "Sensitivity", "Event Studies", "Regression", "Risk", "Methodology"])

    with research_tabs[0]:
        st.subheader("Findings and Current Claim")
        st.write(
            "The app remains a prototype for long-run resale time-series analysis, but the Retail Scarcity Score now has an independently tested empirical result. "
            "RSS is calculated without resale price or return information and shows a moderate positive relationship with excess CAGR in the cleaned 70-item benchmark sample."
        )
        st.markdown("""
**What is now supported by the validated scarcity test**
- All 116 tracked products receive a reproducible Retail Scarcity Score from documented distribution, access, and replenishment structure.
- The score passed 32/32 pre-specified ordering checks and remained highly stable under major component-weight shifts.
- In the cleaned 70-item S&P benchmark sample, RSS is moderately positively associated with excess CAGR (Pearson r ≈ 0.449; Spearman ρ ≈ 0.454).

**What this does not prove**
- Scarcity does not automatically cause higher returns. Brand, category, release timing, demand shocks, and other omitted variables can affect performance.
- The broader historical index, correlation, regression, and projection views still rely partly on prototype-estimated resale histories.
- Investability weights remain a proposed scoring framework rather than an optimized investment rule.

**Research framing**
The strongest current methodological contribution is the Retail Scarcity Score and its validation. The broader dashboard should still be presented as a prototype framework until more verified sold-price histories replace simulated paths.
""")

    with research_tabs[1]:
        st.subheader("Retail Scarcity Validation")
        st.write(
            "Retail Scarcity Score (RSS) uses only documented retail release structure: 45% distribution restriction, 20% access restriction, and 35% replenishment scarcity. "
            "If replenishment cannot yet be fairly observed, its weight is removed and D/A are renormalized instead of guessing."
        )

        scarcity_sample = market[
            market.get("benchmark_sample", pd.Series(index=market.index, dtype=str)).astype(str).str.lower().eq("yes")
            & market["excess_cagr"].notna()
            & market["retail_scarcity_score"].notna()
        ].copy() if "excess_cagr" in market.columns else pd.DataFrame()

        if len(scarcity_sample) >= 10:
            pearson_r = scarcity_sample["retail_scarcity_score"].corr(scarcity_sample["excess_cagr"])
            spearman_rho = scarcity_sample["retail_scarcity_score"].rank(method="average").corr(
                scarcity_sample["excess_cagr"].rank(method="average")
            )
            sorted_sample = scarcity_sample.sort_values("excess_cagr", ascending=False)
            top10_avg = sorted_sample.head(10)["retail_scarcity_score"].mean()
            bottom10_avg = sorted_sample.tail(10)["retail_scarcity_score"].mean()

            m1, m2, m3, m4 = st.columns(4)
            m1.metric("Clean benchmark sample", f"{len(scarcity_sample)} items")
            m2.metric("Pearson r", f"{pearson_r:.3f}", help="Final RSS production sample p ≈ 0.000096")
            m3.metric("Spearman ρ", f"{spearman_rho:.3f}", help="Final RSS production sample p ≈ 0.000080")
            m4.metric("Top–bottom RSS gap", f"{top10_avg - bottom10_avg:.2f} points")

            plot_df = scarcity_sample[["item", "category", "retail_scarcity_score", "excess_cagr"]].copy()
            plot_df["Excess CAGR (%)"] = plot_df["excess_cagr"] * 100
            fig_rss = px.scatter(
                plot_df,
                x="retail_scarcity_score",
                y="Excess CAGR (%)",
                color="category",
                hover_name="item",
                title="Retail Scarcity vs Excess CAGR — Clean 70-Item Sample",
            )
            xvals = plot_df["retail_scarcity_score"].astype(float).values
            yvals = plot_df["Excess CAGR (%)"].astype(float).values
            if len(xvals) >= 2 and np.ptp(xvals) > 0:
                slope, intercept = np.polyfit(xvals, yvals, 1)
                xline = np.linspace(float(np.min(xvals)), float(np.max(xvals)), 100)
                fig_rss.add_trace(go.Scatter(
                    x=xline, y=slope * xline + intercept, mode="lines", name="Linear fit"
                ))
            fig_rss.add_hline(y=0, line_width=1, opacity=0.5)
            fig_rss.update_layout(height=560, xaxis_title="Retail Scarcity Score (RSS)", yaxis_title="Excess CAGR vs S&P 500 (%)")
            st.plotly_chart(fig_rss, use_container_width=True)
            st.caption("Using the final 116-item RSS values, the clean-70 sample gives Pearson r ≈ 0.449 (p ≈ 0.000096) and Spearman ρ ≈ 0.454 (p ≈ 0.000080). Association does not establish causation.")

            top_bottom = pd.DataFrame({
                "Group": ["Top 10 excess-return items", "Bottom 10 excess-return items"],
                "Average RSS": [top10_avg, bottom10_avg],
            })
            fig_tb = px.bar(top_bottom, x="Group", y="Average RSS", text_auto=".2f", title="Average Retail Scarcity: Top vs Bottom Performers")
            fig_tb.update_layout(height=420, yaxis_range=[0, 100])
            st.plotly_chart(fig_tb, use_container_width=True)

            st.info(
                "Additional validation: 116/116 products scored; 32/32 anchor-order checks passed; "
                "RSS rank correlations remained 0.993–0.996 when major component weights were shifted by about 10 percentage points."
            )
        else:
            st.warning("Clean-70 benchmark fields are missing from the current CSV. Upload the RSS-updated production dataset to reproduce the validation charts.")
...
""")
    ...
    with research_tabs[6]:
        ...
        st.markdown("""
### Research purpose
Hype Asset Index is a prototype framework for testing whether sneakers, streetwear, and luxury resale goods behave like alternative assets. The core variables are resale premium, growth, liquidity, volatility, risk, correlation with traditional assets, and Retail Scarcity Score (RSS).

### Retail Scarcity Score
RSS measures **retail scarcity**, not literal manufacturer production quantity. It is calculated only from documented release structure:
...
Each observed component is coded on a fixed 0/25/50/75/100 rubric. If replenishment is too new to judge, R is withheld and the remaining weights are renormalized. Resale price, premium, CAGR, sales volume, S&P 500 performance, brand prestige, collaboration status, and the legacy scarcity score are excluded from RSS.

The locked 116-item validation scored all 116 products, including 76 high-confidence and 40 medium-confidence rows with no low-confidence estimates. It passed 32/32 pre-specified anchor-order tests. Under major component-weight changes, the resulting rankings remained highly stable (Spearman rank correlations 0.993–0.996 versus the base model).

### Independent performance test
The cleaned 70-item benchmark sample tests RSS against excess CAGR versus the S&P 500. Because RSS contains no price or return data, this test is not mechanically circular. Using the final 116-item RSS values, the clean-70 sample gives Pearson **r ≈ 0.449 (p ≈ 0.000096)** and Spearman **ρ ≈ 0.454 (p ≈ 0.000080)**. This is a moderate positive association, not evidence that scarcity causes returns.

### Data provenance
Market observations remain separated into verified snapshots and low-volume market snapshots. Prototype histories are generated with a fixed random seed for reproducibility, and monthly sale counts in the simulated panel are proxies derived from available sales-volume fields. The RSS evidence basis and confidence fields are stored separately from market-price fields.

### Index construction
Category and market baskets are built from monthly percentage changes in resale values. Baskets default to established items with at least six months of history so brand-new drops do not distort the entire category. These history paths remain prototype-estimated until verified sold-price time series replace them.

### Regression model
The regression uses item fixed effects, category, brand, collaboration, market segment, grail score, Retail Scarcity Score, liquidity, and month effects. The research view reports both in-sample fit and a chronological holdout check, but regression coefficients should still be treated as model-discipline outputs rather than empirical findings while prototype histories remain in the panel.

### Scoring framework
The Investability Score remains a proposed framework rather than an empirically optimized investment rule. Current weights are 40% liquidity, 25% premium over retail, 20% inverse risk, and 15% RSS. Updating all 116 products from the legacy scarcity score to RSS changes Investability but does not rebuild the score around returns.

### Projection framework
The projection tool is illustrative only. It combines recent item trend, category trend, broad market trend, liquidity, risk, Investability, segment, and mean reversion. It should be read as a scenario model, not a price prediction.

### Limitations
RSS captures documented retail scarcity, not exact production quantity or total surviving supply. Scarcity may also be correlated with other omitted product characteristics. The broader dashboard still has incomplete historical transaction data and prototype-estimated histories, so time-series regression, correlation, and event-study views should not be presented as final empirical evidence.
...
with tabs[5]:
    ...
    st.write("Current market dataset with validated Retail Scarcity Score inputs:")
    display_seed = market.copy()
    display_item_table(display_seed, ["item","brand","category","release_year","retail_price","current_resale","sales_volume","distribution_restriction","access_restriction","replenishment_scarcity","replenishment_status","retail_scarcity_score","scarcity_confidence","data_status","source"])
